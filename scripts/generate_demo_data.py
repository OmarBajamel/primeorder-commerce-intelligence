...
def generate() -> Dict[str, List[Dict[str, Any]]]:
    ...
    for day_offset in range(DAY_COUNT):
        ...
        for segment in range(3):
            ...
            customer_type = "returning" if (day_offset + segment) % 4 == 0 else "new"
            ...
            for purchase_index in range(purchases):
                ...
                orders.append({
                    ...
                    "customer_type": customer_type, "channel": channel, "source": source,
            ...
            commerce_row = {
                ...
                "customer_type": customer_type, "sessions": sessions, "users": users,
                "product_views": product_views, "add_to_carts": add_to_carts,
                "begin_checkouts": checkouts, "purchases": purchases,
                ...
                "cost_sar": round2(cost), "ad_spend_sar": ad_spend,
            }
            ...
            ga4_daily.append({
                "date": day.isoformat(), "channel": channel, "source": source, "medium": medium,
                "campaign": campaign, "device": device, "sessions": int(sessions * 0.98),
                "users": int(users * 0.98), "product_views": int(product_views * 0.97),
                "add_to_carts": int(add_to_carts * 0.96), "begin_checkouts": int(checkouts * 0.95),
        ...
        clarity.append({
            ...
            "sessions": int(sum(row["sessions"] for row in commerce[-3:]) * 0.91),
    ...
    return {
        ...
        "order_items": order_items, "ga4_daily": ga4_daily, "events": events,
...
def build_api(data: Dict[str, List[Dict[str, Any]]]) -> Dict[str, Any]:
    ...
    totals = {key: sum(float(r[key]) for r in commerce) for key in [
        "sessions", "users", "product_views", "add_to_carts", "begin_checkouts", "purchases",
        "units_sold", "gross_revenue_sar", "discount_sar", "refund_sar", "net_revenue_sar", "cost_sar", "ad_spend_sar",
    ]}
    totals.update({
        "average_order_value_sar": round2(totals["net_revenue_sar"] / totals["purchases"]),
    ...
    for row in commerce:
        for key in ["sessions", "purchases", "net_revenue_sar"]:
            by_day[row["date"]][key] += float(row[key])
    ...
    for row in commerce:
        ...
        for key in ["sessions", "purchases", "net_revenue_sar", "ad_spend_sar"]:
    ...
    for keys, values in sorted(acquisition_rollup.items()):
        acquisition_items.append({
            ...
            "sessions": int(values["sessions"]), "purchases": int(values["purchases"]),
            "net_revenue_sar": round2(values["net_revenue_sar"]), "ad_spend_sar": round2(values["ad_spend_sar"]),
            "conversion_rate": round(values["purchases"] / values["sessions"], 6),
            "roas": round(values["net_revenue_sar"] / values["ad_spend_sar"], 4) if values["ad_spend_sar"] else None,
        })
    ...
    customer_rollup: Dict[str, Dict[str, float]] = defaultdict(lambda: defaultdict(float))
    for row in commerce:
        bucket = customer_rollup[row["customer_type"]]
        for key in ["sessions", "purchases", "net_revenue_sar"]:
            bucket[key] += float(row[key])
    customers = [{
        "customer_type": kind, "sessions": int(values["sessions"]), "purchases": int(values["purchases"]),
    ...
    event_total = sum(int(event["event_count"]) for event in data["events"])
    parameter_names = [
    ...
    parameter_completeness = min(
        sum(float(event[name]) * int(event["event_count"]) for event in data["events"]) / event_total
    ...
    quality_checks = [
        {"check_id": "duplicate_transactions", "status": "warning", "severity": "high", "metric_value": round(duplicate_rows / len(data["orders"]), 6), "threshold": 0.0, "affected_rows": duplicate_rows},
        {"check_id": "unknown_products", "status": "warning", "severity": "high", "metric_value": round(unknown_rows / len(data["order_items"]), 6), "threshold": 0.0, "affected_rows": unknown_rows},
        {"check_id": "event_parameter_completeness", "status": "warning", "severity": "medium", "metric_value": round(parameter_completeness, 6), "threshold": 1.0, "affected_rows": 1},
        {"check_id": "search_freshness_days", "status": "warning", "severity": "medium", "metric_value": 11.0, "threshold": 7.0, "affected_rows": 11},
        {"check_id": "purchase_reconciliation", "status": "warning" if reconciliation_affected_days else "pass", "severity": "medium", "metric_value": round(max(daily_variances.values()), 6), "threshold": 0.10, "affected_rows": reconciliation_affected_days},
        {"check_id": "consent_state_coverage", "status": "warning", "severity": "low", "metric_value": min(float(event["consent_state_coverage"]) for event in data["events"]), "threshold": 0.95, "affected_rows": len({event["date"] for event in data["events"] if float(event["consent_state_coverage"]) < 0.95})},
    ]
    insights = [
        ...
        {"insight_id": "INS-002", "priority": 2, "area": "funnel", "title": "Investigate checkout abandonment", "evidence": f"{1 - totals['purchases'] / totals['begin_checkouts']:.1%} synthetic checkout-to-purchase abandonment", "confidence": "medium", "recommended_action": "Segment checkout friction by device and payment method, then test one change."},
    ...
    return {
        ...
        "connectors": [
            ...
            for name, display, formats in [
                ("salla_mcp", "PrimeOrder / Salla MCP", ["json"]),
...
def build_dashboard(data: Dict[str, List[Dict[str, Any]]], api: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for item in data["order_items"]:
        ...
        group = record_groups.setdefault(key, {
            ...
            "product": product["product_id"], "sessions": 0, "users": 0, "viewItem": 0,
            "addToCart": 0, "beginCheckout": 0, "orders": 0, "units": 0,
    ...
    segment_groups: Dict[tuple, List[Dict[str, Any]]] = defaultdict(list)
    for key, group in record_groups.items():
        segment_groups[key[:3]].append(group)
    for commerce in data["commerce_daily"]:
        matching = segment_groups[(commerce["date"], commerce["device"].title(), commerce["channel"])]
        weights = [group["orders"] for group in matching]
        for field, target in [
            ("sessions", "sessions"), ("users", "users"), ("product_views", "viewItem"),
            ("add_to_carts", "addToCart"), ("begin_checkouts", "beginCheckout"),
        ]:
            for group, allocated in zip(matching, _allocate_integer(int(commerce[field]), weights)):
                group[target] += allocated
    ...
    organic = [row for row in data["commerce_daily"] if row["channel"] == "Organic Search"]
    organic_totals = {
        "sessions": sum(int(row["sessions"]) for row in organic),
        "orders": sum(int(row["purchases"]) for row in organic),
        "revenue": sum(float(row["net_revenue_sar"]) for row in organic),
    }
    click_total = sum(values["clicks"] for values in page_rollup.values())
    landing_pages = []
    for page, values in sorted(page_rollup.items(), key=lambda pair: pair[1]["clicks"], reverse=True):
        share = values["clicks"] / click_total if click_total else 0
        landing_pages.append({
            "page": page, "clicks": int(values["clicks"]),
            "sessions": round(organic_totals["sessions"] * share),
            "orders": round(organic_totals["orders"] * share),
            "revenue": round2(organic_totals["revenue"] * share),
        })
    issue_names = {
    ...
    merchant_diagnostics = [{
        "issue": issue_names[issue], "affected": affected,
    ...
    for order in data["orders"]:
        ...
        customer_types[customer] = order["customer_type"]
    ...
    connector_counts = {
        ...
        "google_ads": sum(row["channel"] in {"Paid Search", "Paid Social"} for row in data["commerce_daily"]),
    }
    connectors = [{
    ...
    quality_copy = {
        ...
        "purchase_reconciliation": ({"en": "Salla versus GA4 reconciliation", "ar": "مطابقة سلة مع GA4"}, {"en": "Investigate daily variance above ten percent.", "ar": "فحص التباين اليومي الذي يتجاوز عشرة بالمئة."}),
    ...
    return {
        ...
        "quality": {"healthScore": health_score, "connectors": connectors, "rules": rules, "reconciliation": reconciliation},
        "insights": insights,
    }
def main() -> None:
    ...
    fields = {
        ...
        "commerce_daily": ["date", "channel", "source", "medium", "campaign", "device", "city", "payment_method", "customer_type", "sessions", "users", "product_views", "add_to_carts", "begin_checkouts", "purchases", "units_sold", "gross_revenue_sar", "discount_sar", "refund_sar", "net_revenue_sar", "cost_sar", "ad_spend_sar"],
        "orders": ["order_id", "tracking_transaction_id", "order_date", "anonymous_customer_id", "customer_type", "channel", "source", "medium", "campaign", "device", "city", "payment_method", "coupon_group", "order_status", "gross_revenue_sar", "discount_sar", "refund_sar", "net_revenue_sar"],
    ...
    for path in sorted(output.rglob("*")):
        if path.is_file() and path.name != "manifest.json":
            manifest_files[str(path.relative_to(output)).replace("\\", "/")] = hashlib.sha256(path.read_bytes()).hexdigest()
    metadata = {
        ...
        "row_counts": {name: len(rows) for name, rows in data.items()}, "sha256": manifest_files,
    }
