...
const formula = {
  ...
  conversion: "GA4 tracked purchases ÷ GA4 sessions", aov: "Net revenue ÷ completed orders", refund: "Refunded value ÷ gross revenue",
};

function useMetrics() {
  const { data, filteredRecords: records, filters } = useDashboard();
  return useMemo(() => {
    const gross = sum(records, "revenue"), refunds = sum(records, "refunds"), orders = sum(records, "orders"), sessions = sum(records, "sessions"), trackedPurchases = sum(records, "trackedPurchases");
    ...
    const previousGross = sum(previous, "revenue") - sum(previous, "refunds"), previousOrders = sum(previous, "orders"), previousSessions = sum(previous, "sessions"), previousTrackedPurchases = sum(previous, "trackedPurchases");
    const pct = (current: number, prior: number) => prior ? ((current - prior) / prior) * 100 : null;
    const net = gross - refunds, conversion = sessions ? trackedPurchases / sessions * 100 : 0, aov = orders ? net / orders : 0;
    return { records, gross, net, refunds, orders, sessions, trackedPurchases, activeUserDays: sum(records, "activeUserDays"), units: sum(records, "units"), conversion, aov,
      refundRate: gross ? refunds / gross * 100 : 0,
      changes: { revenue: pct(net, previousGross), orders: pct(orders, previousOrders), conversion: pct(conversion, previousSessions ? previousTrackedPurchases / previousSessions * 100 : 0) } };
...
function Overview() {
  ...
  return <><PageIntro eyebrow="DECISION SNAPSHOT" title={t("overview")} description={locale === "ar" ? "قراءة موحدة للأداء التجاري وموثوقية القياس والفرص التالية." : "A unified view of commercial performance, measurement trust and the next best actions."} />
    ...
    <div className="grid wide-left"><SectionCard title={t("revenueTrend")}><TrendChart data={trends} label="Weekly net revenue and completed orders" /></SectionCard><SectionCard title={t("health")}><div className="score-wrap"><div className="score-ring" style={{ "--score": `${data.quality.healthScore * 3.6}deg` } as React.CSSProperties}><strong>{data.quality.healthScore}</strong><span>/ 100</span></div><div><StatusChip status="MONITOR" /><p>{locale === "ar" ? `${data.quality.rules.filter((rule) => rule.status !== "pass").length} قواعد تحتاج إلى الإصلاح قبل اتخاذ قرارات دقيقة.` : `${data.quality.rules.filter((rule) => rule.status !== "pass").length} rules need remediation before precision decisions.`}</p><a href="quality/">{t("viewDetails")} <ArrowRight size={14} /></a></div></div></SectionCard></div>
...
function Funnel() {
  ...
  const values = [m.sessions, sum(m.records, "viewItem"), sum(m.records, "addToCart"), sum(m.records, "beginCheckout"), m.trackedPurchases];
  ...
  const devices = groupRecords(m.records, "device").map((row) => ({ name: row.name, value: row.sessions ? row.trackedPurchases / row.sessions * 100 : 0 }));
  const channels = groupRecords(m.records, "channel").map((row) => ({ name: row.name, value: row.sessions ? row.trackedPurchases / row.sessions * 100 : 0 })).sort((a, b) => b.value - a.value);
  return <><PageIntro eyebrow="BEHAVIOR FLOW" title={t("funnelTitle")} description={t("funnelDesc")} /><div className="kpi-grid compact-kpis"><KpiCard label={t("sessions")} value={formatNumber(m.sessions, locale)} formula="SUM(GA4 source-reported sessions)" /><KpiCard label="view_item" value={formatNumber(values[1], locale)} formula="GA4 sessions containing ≥1 view_item event" /><KpiCard label="tracked purchase" value={formatNumber(m.trackedPurchases, locale)} formula="GA4 valid purchase events" tone="teal" /><KpiCard label={t("conversion")} value={formatPercent(m.conversion, locale)} formula={formula.conversion} /></div>
...
function Products() {
  ...
  const rows = groupRecords(m.records, "product").map((row) => ({ ...row, net: row.revenue - row.refunds, refundRate: row.revenue ? row.refunds / row.revenue * 100 : 0, conversion: row.sessions ? row.trackedPurchases / row.sessions * 100 : 0 }))
...
function Acquisition() {
  const { data, locale, t } = useDashboard(); const m = useMetrics(); if (!data) return null;
  const channels = groupRecords(m.records, "channel").map((row) => ({ ...row, net: row.revenue - row.refunds, conversion: row.sessions ? row.trackedPurchases / row.sessions * 100 : 0 })).sort((a, b) => b.net - a.net);
  return <><PageIntro eyebrow="DEMAND SOURCES" title={t("acquisitionTitle")} description={locale === "ar" ? "قارن جودة الجلسات والقيمة التجارية عبر القنوات دون مبالغة في الإسناد." : "Compare session quality and commercial value across channels without overstating attribution."} />
    <SectionCard title={t("sourceMedium")}><div className="table-wrap"><table><thead><tr><th scope="col">{t("channel")}</th><th scope="col">{t("sessions")}</th><th scope="col">{t("users")}</th><th scope="col">Tracked purchases</th><th scope="col">{t("orders")}</th><th scope="col">{t("revenue")}</th><th scope="col">{t("conversion")}</th></tr></thead><tbody>{channels.map((row) => <tr key={row.name}><th scope="row"><b>{row.name}</b><small>{row.name.toLowerCase().replace(" ", "_")} / attributed</small></th><td>{formatNumber(row.sessions, locale)}</td><td>{formatNumber(row.activeUserDays, locale)}</td><td>{formatNumber(row.trackedPurchases, locale)}</td><td>{formatNumber(row.orders, locale)}</td><td>{formatCurrency(row.net, locale)}</td><td>{formatPercent(row.conversion, locale)}</td></tr>)}</tbody></table></div></SectionCard>
    <div className="grid halves"><SectionCard title="Channel revenue"><ComparisonChart data={channels.map((r) => ({ name: r.name, revenue: r.net }))} label="Net revenue by acquisition channel" /></SectionCard><SectionCard title="Tracked purchase rate"><ComparisonChart data={channels.map((r) => ({ name: r.name, value: r.conversion }))} dataKey="value" label="GA4 tracked purchase conversion by channel" /></SectionCard></div><Notice>Active user-days are summed daily active users, not a distinct period-user count.</Notice><Notice>{t("attributionCaveat")}</Notice></>;
}
...
function Seo() {
  ...
  return <><PageIntro eyebrow="ORGANIC DISCOVERY" title={t("seoTitle")} description={locale === "ar" ? "اجمع طلب البحث مع جودة الكتالوج لتحديد فرص قابلة للتحقق." : "Connect search demand with catalog health to surface verifiable opportunities."} /><div className="kpi-grid compact-kpis"><KpiCard label={t("clicks")} value={formatNumber(totals.clicks, locale)} formula="SUM(Search Console clicks)" tone="teal" /><KpiCard label={t("impressions")} value={formatNumber(totals.impressions, locale)} formula="SUM(Search Console impressions)" /><KpiCard label={t("ctr")} value={formatPercent(totals.impressions ? totals.clicks / totals.impressions * 100 : 0, locale)} formula="Clicks ÷ impressions" /><KpiCard label={t("position")} value={(data.seo.queries.reduce((n, q) => n + q.position * q.impressions, 0) / totals.impressions).toFixed(1)} formula="Impression-weighted average position" /></div>
    ...
    <div className="grid halves"><SectionCard title={t("landingPages")}><div className="compact-list">{data.seo.landingPages.map((row) => <div key={row.page}><code>{row.page}</code><span><b>{formatNumber(row.clicks, locale)} {t("clicks")}</b><small>{formatNumber(row.impressions, locale)} {t("impressions")} · {formatPercent(row.ctr, locale)}</small></span></div>)}</div></SectionCard><SectionCard title={t("merchant")}><div className="status-list diagnostics">{data.seo.merchantDiagnostics.map((item) => <div key={item.issue.en}><span><b>{localize(item.issue, locale)}</b><small>{item.affectedItemSnapshots} affected item-snapshots</small></span><StatusChip status={item.status} /></div>)}</div></SectionCard></div></>;
}
...
function Methodology() {
  ...
  const cards = locale === "ar" ? [
    ...
    [t("privacy"), "لا توجد بيانات شخصية حقيقية. عقد لوحة المعلومات المنشور لا يتضمن معرّفات على مستوى الصف، ولا يتصل بواجهة خاصة أو مصدر تاجر."],
    ...
    [t("kpiMethod"), "Commerce owns completed orders and net revenue. GA4 owns sessions and the behavioral funnel; tracked purchases divided by sessions defines conversion. Active users are shown only as additive user-days."],
    ...
    [t("privacy"), "No real personal data exists. The published dashboard contract contains no row-level identifiers and never calls a private API or merchant connector."],
  ...
  return <><PageIntro eyebrow="EVIDENCE & LIMITS" title={t("methodologyTitle")} description={locale === "ar" ? "تعريفات وحدود ومصادر واضحة حتى يتمكن القارئ من الحكم على الأدلة." : "Clear definitions, boundaries and provenance so every reader can judge the evidence."} /><div className="method-hero"><div><span className="eyebrow">{t("transparent")}</span><h2>public-demo <span>≠</span> live-private</h2><p>{t("synthetic")}</p></div><div className="lineage"><span>Deterministic fixtures</span><ArrowRight /><span>Validated KPI export</span><ArrowRight /><span>Static JSON</span><ArrowRight /><span>Dashboard</span></div></div><div className="method-grid">{cards.map(([title, body], index) => <article key={title}><span>{icons[index]}</span><h2>{title}</h2><p>{body}</p></article>)}</div><SectionCard title="Dashboard contract v1.0"><div className="contract"><code>public/data/dashboard.json</code><p>Required: <code>meta.dataMode = &quot;public-demo&quot;</code>. Product behavior comes from the independent GA4 product fixture; product sales come from order items. Records expose sessions, active user-days, funnel events, tracked purchases, completed orders, units, revenue and refunds. DuckDB/dbt independently validates the same seeds and semantics.</p></div></SectionCard></>;
}
