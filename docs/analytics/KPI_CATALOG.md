...
| `commerce.revenue_per_session` Revenue per session | `net_revenue / sessions` in the current synthetic mart; live behavioral variant uses `GA4 purchase_revenue / GA4 sessions` | Public conformed fixture; live `GA4` or explicitly `RECONCILED` | Date × acquisition/device using one named measurement plane | The current fixture carries conformed sessions with commerce measures. A live merchant-revenue/GA4-session variant must be explicitly cross-source. | `AVAILABLE_PUBLIC_DEMO` |
...
| ID / KPI | Formula | Source priority | Filters and grain | Caveats | Availability |
|---|---|---|---|---|---|
| `acquisition.users` Users | Source-reported distinct users | `GA4` | Selected period × channel/source/medium/campaign/device | Public-demo is synthetic. Live users are non-additive across dimension rows/dates; identity/thresholding apply. | `AVAILABLE_PUBLIC_DEMO` |
