with metrics as (
    select
        ...
        (select 1 - sum(tracked_purchases)::double / nullif(sum(begin_checkouts), 0) from {{ ref('int_daily_ga4') }}) as checkout_abandonment
)
...
union all
select 'INS-002', 2, 'funnel', 'Investigate checkout abandonment', concat(round(checkout_abandonment * 100, 1), '% GA4 checkout-to-purchase abandonment'),
